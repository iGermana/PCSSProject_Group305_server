...
namespace TcpEchoServer
{
    ...
    public static class Globals
    {
        ...
        public static bool sixPlayersOnline { get; set; }
    ...
    class ClientHandler
    {
        ...
        int count = 0;
        string[] roles = { "president", "chancellor", "none" };
        ...
        public void handling()
        {
            ...
            try
            {
                while (true)
                {
                    ...
                    if (Globals.sixPlayersOnline == false)
                    {
                        stream.Flush();
                        outputData = Encoding.ASCII.GetBytes("waiting for " + (6-clientCounter).ToString() + " more players to join");
                        stream.Write(outputData, 0, outputData.Length);

                        if (clientCounter == 6)
                        {
                            Globals.sixPlayersOnline = true;
                        }
                    }

                    if (Globals.sixPlayersOnline == true)
                    {
                        ...
                        if (playerName == "player" + (Globals.currentPresident+1).ToString())
                        {
                            stream.Flush();
                        ...
                        {
                            stream.Flush();
                            outputData = Encoding.ASCII.GetBytes("you have not been assigned a role");
                            stream.Write(outputData, 0, outputData.Length);
